...
with col_bg:
    ...
    st.markdown("""
    모바일 게임 시장에서 **확률형 시스템(Gacha)**은 핵심 콘텐츠이지만,  
    단순한 수치 표만으로는 다음과 같은 한계가 존재합니다.

    - **직관성 부족:** 복잡한 확률 구조를 한눈에 이해하기 어려움
    - **체감 확률의 괴리:** 유저가 실제로 느끼는 획득 확률 분석의 부재
    - **경제적 영향성:** 가챠 시스템이 게임 환경에 미치는 영향 파악 난해
...
with col_pur:
    ...
    st.markdown("""
    **스톤에이지 키우기**의 실제 확률 데이터를 기반으로  
    차별화된 인사이트를 제공하는 것을 목적으로 합니다.
