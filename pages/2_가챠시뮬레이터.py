...
if st.button("🚀 시뮬레이션 시작"):
    with st.spinner('Monte Carlo 시뮬레이션 진행 중... (대량 데이터 생성 중)'):
        ...
        if rare_grades:
            ...
            with col_a:
                ...
                **{selected_grade} 등급 결과:**
                ...
                - 최대 획득: {df_sim[selected_grade].max()}회
                
                평균적으로는 {avg_hits:.2f}개를 얻지만, 운이 좋은 유저는 {df_sim[selected_grade].max()}개까지 얻는 것을 확인할 수 있습니다.
                """)
            
            with col_b:
                fig_dist = px.histogram(df_sim, x=selected_grade, 
                                    title=f"{n_draws}회 뽑기 시 {selected_grade} 획득 빈도 분포 ({n_trials}명 시뮬레이션)",
                                    nbins=range(int(df_sim[selected_grade].max()) + 2),
                                    labels={selected_grade: '획득 횟수(Hits)', 'count': '유저 수(Users)'},
    ...
    st.write("사이드바에서 설정을 완료한 후 **시뮬레이션 시작** 버튼을 눌러주세요.")
